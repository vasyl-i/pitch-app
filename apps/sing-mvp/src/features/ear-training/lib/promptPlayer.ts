...
export function noteScript(midi: number, duration = 1.5): PromptScript {
  return script([{ midis: [midi], at: 0, duration, volume: 0.3 }]);
}
...
export function chordScript(midis: number[], blockDuration = 1.6): PromptScript {
  const arpStep = 0.28;
  const tones: PromptTone[] = midis.map((m, i) => ({ midis: [m], at: i * arpStep, duration: 0.32, volume: 0.26 }));
  tones.push({ midis, at: midis.length * arpStep + 0.15, duration: blockDuration, volume: 0.2 });
  return script(tones);
}
...
export function melodyScript(notes: { midi: number; start: number; duration: number }[]): PromptScript {
  return script(notes.map((n) => ({ midis: [n.midi], at: n.start, duration: n.duration, volume: 0.26 })));
}
